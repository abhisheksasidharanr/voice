...
import {functionMap} from  "../lib/constants"
  ...
  useEffect(() => {
    if (microphoneState === 1 && socket && defaultStsConfig) {
      ...
      const onOpen = () => {
        ...
        setTimeout(() => {
          if (pathname === "/") {
            toggleSleep(); // triggers assistant welcome message
            startMicrophone();
        startListening(true);
          }
        }, 1500);
    ...
    <div className={className}>
      ...
      {!microphone ? (
        ...
        <Fragment>
          ...
          {socketState === 0 && (
            <div className="text-base text-gray-25 text-center w-full">Preparing.....</div>
          )}
